...
class USBIPComponent : public Component {
 public:
  ...
  bool is_device_connected() const { return this->usb_device_.connected; }
  uint16_t get_device_vid() const { return this->usb_device_.vid; }
  uint16_t get_device_pid() const { return this->usb_device_.pid; }
  std::string get_device_info() const {
    if (!this->usb_device_.connected) {
      return "Disconnected";
    }
    char buf[128];
    if (this->usb_device_.vid == 0x03F0) {
      if (this->usb_device_.pid == 0x0C17) {
        return "HP LaserJet 1010 (Ready)";
      } else if (this->usb_device_.pid == 0x0317) {
        return "HP LaserJet 1010 (Bootloader)";
      } else {
        sprintf(buf, "HP Device (VID: 0x03F0, PID: 0x%04X)", this->usb_device_.pid);
        return buf;
      }
    }
    sprintf(buf, "Unknown Device (VID: 0x%04X, PID: 0x%04X)", this->usb_device_.vid, this->usb_device_.pid);
    return buf;
  }

  std::string get_client_status() const {
    switch (this->client_state_) {
      case CLIENT_STATE_IDLE:
        return "Idle (No Connection)";
      case CLIENT_STATE_DEVLIST:
        return "Querying device list";
      case CLIENT_STATE_ATTACHED:
